...
window.addEventListener("beforeunload", () => {
  ...
  if (_userMarker && _locationLayer) _locationLayer.clearLayers();
  const client = sb();
  if (client) {
...
function setView(view) {
  ...
  if (view === "map") {
    ...
    setTimeout(() => {
      _map.invalidateSize();
      render();
      setTimeout(() => initLocationTracking(), 100);
    }, DELAY_MAP_RENDER);
  }
}
...
function initLocationTracking() {
  ...
  console.log("Iniciando rastreamento de GPS");
  _geoWatcher = navigator.geolocation.watchPosition(onLocationUpdate, null, {
...
function onLocationUpdate(pos) {
  ...
  _userMarker = L.marker([lat, lng], { icon: makeLocationIcon("#3498db") }).addTo(_locationLayer);
